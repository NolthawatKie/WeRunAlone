...
interface ShareModalProps {
  planName: string;
  onConfirm: (planName: string, sharedBy: string) => Promise<void>;
  onClose: () => void;
}

export default function ShareModal({ planName, onConfirm, onClose }: ShareModalProps) {
  const [customPlanName, setCustomPlanName] = useState(planName);
  ...
  const handleSubmit = async () => {
    ...
    if (!agreed) {
      setError('Please confirm this plan was generated by WeRunAlone.');
      return;
    }
    setLoading(true);
    setError('');
    try {
      await onConfirm(customPlanName.trim(), name.trim() || 'Anonymous Runner');
    } catch (err) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-white rounded-xl border border-slate-200 shadow-lg w-full max-w-md p-6 z-10">
        ...
        <p className="text-sm text-slate-500 mb-5">
          Let others discover and use your training plan.
        </p>
